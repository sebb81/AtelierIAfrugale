...
class HandGestureVideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if self._last_frame_time is not None:
            ...
            if dt > 0:
                fps_instant = 1.0 / dt
                # EMA leger pour stabiliser l'affichage du FPS
                self._fps = 0.9 * self._fps + 0.1 * fps_instant
    ...
    if WEBRTC_AVAILABLE:
        ...
        with col_right:
            ctx = webrtc_streamer(
                ...
                "width": {"min": 424, "ideal": 424, "max": 424},
                "height": {"min": 240, "ideal": 240, "max": 240},
                "frameRate": {"min": 15, "ideal": 15, "max": 15},
            }, "audio": False},
                video_receiver_size=(424, 240),
        ...
        with col_left:
            ...
            frame_size_placeholder = st.empty()
            ...
            def _live_status():
                ...
                if ctx.state.playing and ctx.video_processor:
                    threshold = st.session_state.get("mission1_threshold", DEFAULT_THRESHOLD)
                    st.session_state.setdefault("mission1_threshold", threshold)
                    ...
                    frame_size = ctx.video_processor.get_frame_size()
                else:
                    fps = 0.0
                    frame_size = (0, 0)
                ...
                fps_placeholder.write(f"FPS (flux recu) : **{fps:.1f}**")
                frame_size_placeholder.write(f"Taille frame : **{frame_size[0]}x{frame_size[1]}**")
                state_placeholder.write("WebRTC: " f"playing={getattr(state, 'playing', None)} " f"ice={getattr(state, 'ice_connection_state', None)} " f"conn={getattr(state, 'connection_state', None)} " f"sig={getattr(state, 'signaling_state', None)}")
